...
export async function receivableRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/:id/distribute', async (request, reply) => {
    try {
      ...
      const totalQuotas = receivable.fund.orders.reduce((sum: number, order: any) => sum + order.quantity, 0);
      const totalToPay = receivable.paidValue || 0;

      const distributions = receivable.fund.orders.map((order: any) => ({
